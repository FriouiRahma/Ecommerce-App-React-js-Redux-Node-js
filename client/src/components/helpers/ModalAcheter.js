...
/***************************FR**************************************************/
const ModalAcheter = ({ commanderfromsection, idofprod, prodr, iduser, fraislivrai, boutiqueid, setAlert, isAuthenticated }) => {
  let userid = iduser;
  const customStyles = {
    content: {
      top: "50%",
    ...
    <div>
      <div>
        ...
        <Modal isOpen={modalIsOpen} onAfterOpen={afterOpenModal} onRequestClose={closeModal} style={customStyles} contentLabel="Example Modal" ariaHideApp={false}>
          <div className="ModalAcheter-Box">
            ...
            <form>
              <div>
                <div style={{ display: "flex", marginBottom: "40px" }}>
                  <img style={{ width: "150px" }} src={`${data.backUrl}/uploads/${prodr && prodr !== null && prodr.photo[0]}`} alt="" />
                  <div className="prod-info">
                    <div className="name-produit">
                      {prodr && prodr !== null && prodr.name}
                      {prodr && prodr !== null && prodr.tarifpromo ? (
                        <div className="prix">{prodr && prodr !== null && prodr.tarifpromo} DT</div>
                      ) : (
                        <div className="prix">{prodr && prodr !== null && prodr.price} DT </div>
                      )}
                    </div>

                    <div>
                      <button className={count === 1 ? "disable-button" : "btn-shop-left"} onClick={(e) => handprodrdecre(e)}>
                        -
                      </button>
                ...
                <div className="enter-coord">Entrer vos coordonnés</div>
...
export default connect(mapStateToProps, { commanderfromsection, setAlert })(ModalAcheter);
